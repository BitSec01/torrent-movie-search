...
export function OrganizePage() {
  ...
  const handleOrganize = async () => {
    setOrganizing(true);
    setLogs([{ time: new Date().toISOString(), action: "START", detail: "Starting organization..." }]);
    try {
      const res = await fetch("/api/library/organize-storage", { method: "POST" });
      const data = await res.json();
      if (data.logs) {
        setLogs(data.logs);
      }
  ...
  const torrentsCount = tree?.torrents?.length ?? 0;
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-zinc-500">
                View and organize files in /mnt/storage
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              {torrentsCount > 0 && (
                <button
                  ...
                  disabled={organizing}
                  className="flex items-center gap-2 rounded-lg bg-purple-600/20 px-4 py-2 text-sm font-medium text-purple-400 transition hover:bg-purple-600/30 disabled:opacity-50"
                >
                  {organizing ? (
                    <>
                    ...
                    <>
                      ...
                      AI Organize Torrents ({torrentsCount})
                    </>
                  )}
                </button>
              )}
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {(["Movies", "Series", "torrents"] as const).map((section) => (
                  <SectionTree
                    key={section}
                    section={section}
                    items={tree[section] || []}
              ...
              <p className="text-sm text-zinc-600">Failed to load storage tree</p>
            )}
          </div>
            ...
            {logs.length === 0 ? (
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-8 text-center">
                <p className="text-sm text-zinc-600">
                  {torrentsCount > 0
                    ? `Click "AI Organize Torrents" to organize ${torrentsCount} item${torrentsCount !== 1 ? "s" : ""} in the torrents folder.`
                    : "Nothing to organize. The torrents folder is empty."}
                </p>
              </div>
            ) : (
              <div className="max-h-[600px] overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-900/50">
...
function LogBadge({ action }: { action: string }) {
  ...
  return (
    <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-bold uppercase ${styles[action] ?? "bg-zinc-700 text-zinc-400"}`}>
      {action}
    </span>
  );
}
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 overflow-hidden">
      <button
        ...
        className="flex w-full items-center gap-2 px-4 py-3 text-left hover:bg-zinc-800/30 transition"
      >
        <svg
        ...
        <span className={`text-sm font-semibold ${sectionColors[section] ?? "text-zinc-300"}`}>
          {section}/
        </span>
      ...
      >
        {isDir ? (
          <>
            ...
            <svg className="h-3.5 w-3.5 shrink-0 text-amber-500/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function FileIcon({ name }: { name: string }) {
  ...
  if (videoExts.includes(ext || "")) {
    return (
      <svg className="h-3.5 w-3.5 shrink-0 text-blue-400/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  if (subExts.includes(ext || "")) {
    return (
      <svg className="h-3.5 w-3.5 shrink-0 text-emerald-400/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.076-4.076a1.526 1.526 0 011.037-.443 48.282 48.282 0 005.68-.494c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
      </svg>
    );
  }
  return (
    <svg className="h-3.5 w-3.5 shrink-0 text-zinc-500/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
